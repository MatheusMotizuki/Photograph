...
private:
    ...
    void setup(const std::string& room, Action action) {
        if (m_running.load()) return;
        m_wsClient = std::make_unique<hv::WebSocketClient>();
        
        // Updated API: onopen takes no parameters
        m_wsClient->onopen = [this, room, action]() {
            if (!room.empty()) {
                nlohmann::json j;
                j["room_id"] = room;
                j["topic"] = (action == Action::Create) ? "room.create" : "room.join";
                m_wsClient->send(j.dump());
            }
        };
        
        // Updated API: onmessage takes only the message string
        m_wsClient->onmessage = [this](const std::string& msg) {
            try {
        ...
        // Updated API: onclose callback
        m_wsClient->onclose = []() {
            std::cout << "❌ Connection closed" << std::endl;
        };
        
        // Updated API: open() takes const char*
        m_wsClient->open(m_url.c_str());
        m_running = true;
    }

    void info(const std::string& room, const ImVec2& cursor) {
        if (!m_running.load() || !m_wsClient || room.empty()) return;
        nlohmann::json msg;
        msg["topic"] = "room.mouse.pos";
        msg["room_id"] = room;
        msg["data"] = { cursor.x, cursor.y };
        m_wsClient->send(msg.dump());
    }
    ...
    std::string m_url;
    std::unique_ptr<hv::WebSocketClient> m_wsClient;
